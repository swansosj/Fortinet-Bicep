...
param subnet5Name string = 'RouteServerSubnet'

@description('Subnet 5 Prefix')
param subnet5Prefix string = '10.0.4.0/28'

@description('Subnet 4 start address, 2 consecutive private IPs are required')
param subnet5StartAddress string = '10.0.4.4'
...
@description('Public IP for Route Server. This deployment uses a Standard SKU Azure Load Balancer and requires a Standard SKU public IP. Microsoft Azure offers a migration path from a basic to standard SKU public IP. The management IP\'s for both FortiGate can be set to none. If no alternative internet access is provided, the SDN Connector functionality for both HA failover and dynamic objects will not work.')
@allowed([
  'new'
  'existing'
  'none'
])
param publicIP4NewOrExisting string = 'new'

@description('Public IP Resource Group, this value is required if an existing Public IP is selected')
param publicIP4ResourceGroup string = ''
...
var publicIP4Id = ((publicIP4NewOrExisting == 'new')
  ? publicIP4.id
  : resourceId(publicIP4ResourceGroup, 'Microsoft.Network/publicIPAddresses', publicIP4NameVar))

var publicIP4NameVar = ((publicIP4Name == '')
  ? '${customerName}-pip-rtserve-prod-${resourceGroup().location}-000'
  : publicIP4Name)
...
var subnet5Id = ((vnetNewOrExisting == 'new')
  ? resourceId('Microsoft.Network/virtualNetworks/subnets', vnetNameVar, subnet5Name)
  : resourceId(vnetResourceGroup, 'Microsoft.Network/virtualNetworks/subnets', vnetNameVar, subnet5Name))
var sn5IPArray = split(subnet5Prefix, '.')
var sn5IPArray2 = string(int(sn5IPArray[2]))
var sn5IPArray1 = string(int(sn5IPArray[1]))
var sn5IPArray0 = string(int(sn5IPArray[0]))
var sn5IPStartAddress = split(subnet5StartAddress, '.')
var sn5IPrsIP1 = '${sn5IPArray0}.${sn5IPArray1}.${sn5IPArray2}.${int(sn5IPStartAddress[3])}'
var sn5IPrsIP2 = '${sn5IPArray0}.${sn5IPArray1}.${sn5IPArray2}.${(int(sn5IPStartAddress[3])+1)}'
...
resource routeServerName_routeServerBgp2 'Microsoft.Network/virtualHubs/bgpConnections@2023-04-01' = {
  parent: routeServer
  name: routeServerBgp2
  properties: {
    peerIp: sn2IPfgb
    peerAsn: fortigateBgpASN
  }
  dependsOn: [
    routeServerName_routeServerIp
    routeServerName_routeServerBgp1
  ]
}
...
resource publicIP4 'Microsoft.Network/publicIPAddresses@2022-05-01' = if (publicIP4NewOrExisting == 'new') {
  name: publicIP4NameVar
  location: location
  tags: {
    'iv-monitoringEnabled': 'false'
  }
